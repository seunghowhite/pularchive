...
/** TiltedCard용 텍스트 모드 배경 (실제 이미지 대신) */
const TILT_TEXT_BACK =
  "data:image/svg+xml," +
  encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" width="800" height="800"><rect fill="#064e3b" width="100%" height="100%"/></svg>`,
  );

/** TiltedCard용 비디오 모드 배경 */
const TILT_VIDEO_BACK =
  "data:image/svg+xml," +
  encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="720"><rect fill="#0a0a0a" width="100%" height="100%"/></svg>`,
  );

const TILT_MODAL_W = "min(90vw, 560px)";
const TILT_MODAL_H = "min(75svh, 620px)";
    ...
    <main className="relative min-h-dvh min-h-[100svh] w-full overflow-hidden font-sans text-emerald-50">
      ...
      {open ? (
        <div className="fixed inset-0 z-50" role="presentation">
          ...
          >
            <div className="pointer-events-auto flex max-h-full max-w-full items-center justify-center overflow-y-auto overscroll-y-contain p-4">
              {kind === "image" ? (
                <TiltedCard
                  imageSrc={payload.image}
                  altText="무작위 이미지"
                  captionText={kindLabel.image}
                  containerHeight={TILT_MODAL_H}
                  containerWidth={TILT_MODAL_W}
                  imageHeight={TILT_MODAL_H}
                  imageWidth={TILT_MODAL_W}
                  rotateAmplitude={12}
                  scaleOnHover={1.05}
              ...
              {kind === "text" ? (
                <TiltedCard
                  imageSrc={TILT_TEXT_BACK}
                  altText=""
                  captionText={kindLabel.text}
                  containerHeight={TILT_MODAL_H}
                  containerWidth={TILT_MODAL_W}
                  imageHeight={TILT_MODAL_H}
                  imageWidth={TILT_MODAL_W}
                  ...
                  overlayContent={
                    <p className="text-pretty text-center text-lg font-medium leading-relaxed text-white sm:text-xl md:text-2xl">
                      {payload.text}
                    </p>
                  }
                />
              ...
              {kind === "video" ? (
                <TiltedCard
                  imageSrc={TILT_VIDEO_BACK}
                  altText=""
                  captionText={kindLabel.video}
                  containerHeight={TILT_MODAL_H}
                  containerWidth={TILT_MODAL_W}
                  imageHeight={TILT_MODAL_H}
                  imageWidth={TILT_MODAL_W}
                  ...
                  overlayContent={
                    <video
                      className="max-h-full w-full rounded-[15px] object-contain"
                      controls
                      playsInline
                      preload="metadata"
                      src={SAMPLE_VIDEO}
